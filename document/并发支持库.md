...
1. <u>**多个线程不同步地同时访问同一 `std::shared_ptr` 对象**</u>
2. <u>**任一线程使用 shared_ptr 的非 const 成员函数**</u>
...
https://www.bilibili.com/video/BV1zD421M71T/

https://www.bilibili.com/video/BV19w4m1a7fD/
...
void worker(int id, std::latch& latch) {
    ...
    // 减少计数并通知
    latch.count_down(); // 默认减少1
    //latch.count_down(2); //显示减少2
}
...
### （2）使用 arrive_and_wait（计数减少1，等待计数为0，不会重置计数，划分了两端区域代码，同步）
...
void worker(int id, std::barrier<>& bar) {
    for (int phase = 0; phase < 3; ++phase) {
        ...
        // 到达屏障并等待其他线程
        bar.arrive_and_wait(); //（计数减少1，等待计数为0，会重置计数，划分了两端区域代码，同步）
...
### （2）使用 arrive 和 wait 分离（arrive_and_wait等价于wait(arrive())）
...
### （3）使用 arrive_and_drop（减少当前计数1，并减少重置计数1）
...
视频介绍 	https://www.bilibili.com/video/BV11s421g7Pu/

## 1. 概述

C++20 引入了信号量（Semaphore）作为新的同步原语，定义在 `<semaphore>` 头文件中。信号量是一种比互斥锁更灵活的同步机制，**<u>它允许一定数量的线程同时访问共享资源</u>**，而不仅仅是互斥访问。
